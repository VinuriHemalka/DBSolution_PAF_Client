...
$(document).on("click", "#btnSave", function(event) {
	...
	// If valid------------------------
	var type = ($("#Researcher_ID").val() == "") ? "POST" : "PUT";
...
$(document).on("click", ".btnRemove", function(event) {
	...
	$.ajax({
		url : "ResearcherAPI",
		type : "DELETE",
		data : "Researcher_ID=" +  event.target.value,
		dataType : "text",
		complete : function(response, status) {
...
$(document).on("click",".btnUpdate",function(event)
		{
			...
			$("#Emai").val($(this).closest("tr").find('td:eq(4)').text());
...
function validateItemForm() {
	...
	// Reseracher Name
	if ($("#Reseracher_name").val().trim() == "") {
		return "Please insert Reseracher Name.";
	}
	...
	if ($("#PhoneNumber").val().trim() == "") {
		return "Please insert PhoneNumber.";
	}
